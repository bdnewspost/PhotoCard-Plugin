 ...
 * - Featured image centered in middle with red border/shadow effect
 * - Image area has slight padding, red shows around image
 * - Title: Large WHITE bold text on red background at bottom
 * - "❮❮ নিউজ লিংক কমেন্টে ❯❯" at very bottom with decorative styling
 * - NO white background anywhere - entire card is RED
 */
if (!defined('ABSPATH')) exit;

$kalbela_bg = isset($options['kalbela_bg_color']) ? $options['kalbela_bg_color'] : '#cc0000';
// Darker shade for subtle depth
$kalbela_dark = $kalbela_bg;
?>
<div class="pcd-photocard" data-language="<?php echo esc_attr($language); ?>" data-quality="<?php echo esc_attr($image_quality); ?>" style="width: 1080px; height: 1080px; background: <?php echo esc_attr($kalbela_bg); ?>; padding: 0; position: relative; display: flex; flex-direction: column; box-sizing: border-box; overflow: hidden;">
    ...
    <div style="padding: 20px 30px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; min-height: 110px;">
        <?php if ($logo_position === 'right'): ?>
            <!-- Date Left, Logo Right -->
            <?php if ($enable_date): ?>
            <div style="color: #ffffff; font-size: 30px; font-weight: 700; font-family: '<?php echo esc_attr($title_font_family); ?>', 'Noto Sans Bengali', sans-serif; text-shadow: 1px 1px 3px rgba(0,0,0,0.3);">
    ...
    <!-- Featured Image Area - on RED background with border effect -->
    <div style="flex: 1; padding: 5px 28px; min-height: 0; overflow: hidden; display: flex; align-items: center; justify-content: center;">
        <div style="width: 100%; height: 100%; position: relative; border-radius: 8px; overflow: hidden; box-shadow: 0 8px 40px rgba(0,0,0,0.4), inset 0 0 0 4px rgba(255,255,255,0.15);">
    ...
    <div style="padding: 22px 35px 8px; flex-shrink: 0;">
    ...
    <!-- Details Button with decorative chevrons -->
    <?php if ($show_details_button): ?>
    <div style="text-align: center; padding: 10px 35px 18px; flex-shrink: 0;">
        <span style="color: rgba(255,255,255,0.95); font-size: 26px; font-weight: 700; font-family: '<?php echo esc_attr($title_font_family); ?>', 'Noto Sans Bengali', sans-serif; letter-spacing: 2px;">
    ...
    <div style="background: rgba(0,0,0,0.15); padding: 12px 25px; display: flex; justify-content: center; align-items: center; gap: 25px; flex-wrap: wrap; flex-shrink: 0;">
